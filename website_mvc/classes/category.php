...
<?php
	...
	class category
	{
		...
		public function get_product_by_cat_byquan(){
			$query = "SELECT * FROM tbl_product WHERE catId='21' order by RAND() LIMIT 4";
			$result = $this->db->select($query);
			return $result;
		}

		public function get_product_by_cat_byao(){
			$query = "SELECT * FROM tbl_product WHERE catId='20' order by RAND() LIMIT 4";
			$result = $this->db->select($query);
			return $result;
		}
