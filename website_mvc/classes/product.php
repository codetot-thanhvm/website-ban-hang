...
<?php
	...
	class product
	{
		...
		public function insert_product($data,$files){
			...
			$uploaded_image = "update/".$unique_image;
		...
		public function getsoluong($id){
			$query = "SELECT * FROM tbl_product where productId = '$id'";
			$result = $this->db->select($query);
			while($sl = $result->fetch_assoc()){
				return $sl['soluong'];
			}
		}

		public function hethang(){
			$query = "SELECT * FROM tbl_product where soluong = 0";
			$result = $this->db->select($query);
			return $result;
		}


		public function donchuaxuly(){
		...
		public function nhapthem($id){
			$query = "SELECT * FROM tbl_product WHERE productId = '$id'";
			$result = $this->db->select($query)->fetch_assoc();
			$image = $result["image"];
			$productName = $result["productName"];
			$query_insert = "INSERT INTO tbl_spnhapkho(id_sp,ten_sp,img_sp) VALUES('$id','$productName','$image')";
			$insert_cart = $this->db->insert($query_insert);
			if($insert_cart){
				$msg = "<span class='error'>Đã thêm sản phẩm vào danh sách nhập</span>";
			}
			return $msg;
		}

		public function dsnhapthem(){
			$query = "SELECT * FROM tbl_spnhapkho";
			$result = $this->db->select($query);
			return $result;
		}
		public function del_nhaphang($id){
			$query = "DELETE FROM tbl_spnhapkho where id_sp = '$id'";
			$result = $this->db->delete($query);
			if($result){
				$alert = "<span class='success'>Xóa sản phẩm thành công</span>";
				return $alert;
		...
		public function del_all_data_spnhapkho(){
			$query = "DELETE FROM tbl_spnhapkho";
			$result = $this->db->delete($query);
		}
		public function update_sl($id,$sl){
			$query = "UPDATE tbl_spnhapkho SET sl_sp = $sl WHERE id_sp = $id";
			$result = $this->db->delete($query);
		}
		public function insert_Chitietnhapkho(){
			$q = "SELECT MAX(id_nhapkho) AS lonnhat FROM tbl_nhapkho";
			$r = $this->db->select($q);
			while($abc = $r->fetch_assoc()){
				$id_nhapkho = $abc['lonnhat'];
			}
			$query = "SELECT * FROM tbl_spnhapkho";
			...
			if($get_product){
				while($result = $get_product->fetch_assoc()){
					...
					$sl = $result['sl_sp'];
					$image = $result['img_sp'];
					$query_order = "INSERT INTO tbl_chitietnhapkho(id_nvl,ten_sp,sl_nvl,id_nhapkho,img_sp) VALUES('$productid','$productName','$sl','$id_nhapkho','$image')";
					$insert_order = $this->db->insert($query_order);
				}
			}
		}

		public function insert_nhapkho($ten_nhapkho){
			$query = "INSERT INTO tbl_nhapkho(ten_nhapkho) VALUES ('$ten_nhapkho')";
			$this->db->insert($query);
		}

		public function dschitiet($id){
			$query = "SELECT * FROM tbl_chitietnhapkho WHERE id_nhapkho = '$id'";
			$result = $this->db->select($query);
			return $result;
		}
		...
		public function them_sl(){
			...
			if($get_product){
				while($result = $get_product->fetch_assoc()){
					$productid = $result['id_sp'];
					$sl = $result['sl_sp'];
					$query_add = "UPDATE `tbl_product` SET `soluong`= (tbl_product.soluong + '$sl') WHERE productId = '$productid'";
					$add_sl = $this->db->insert($query_add);
				}
			}
		}
